...
class KA2DMCSimulatorNumba:
    ...
    def run(self) -> Dict[str, Array]:
        ...
        if self.cfg.n_samples <= 0:
            max_disp = self._run_block(self.cfg.sweeps, beta, max_disp, acc_hist)
            return {
        ...
        while remaining >= si:
            max_disp = self._run_block(si, beta, max_disp, acc_hist, record_sweeps=False)
            # relax_wrapped_list.append(self.pos.copy())
            # relax_real_list.append(self.pos_real.copy())
            remaining -= si
        if remaining > 0:
            # run leftover sweeps and record a final snapshot
            max_disp = self._run_block(remaining, beta, max_disp, acc_hist, record_sweeps=False)
            # relax_wrapped_list.append(self.pos.copy())
            # relax_real_list.append(self.pos_real.copy())
        ...
        samples_pos = np.empty((n_saved, N, 2), dtype=np.float64)
        samples_real_pos = np.empty((n_saved, N, 2), dtype=np.float64)
        ...
        if nS > 0 and si > 0:
            # Run first interval sweep-by-sweep without saving the resulting sample
            for _ in range(si):
                max_disp = self._run_block(1, beta, max_disp, acc_hist,
                                           record_sweeps=True, autotune=sampling_autotune)
            # Remaining samples at the usual interval
            for s_idx in trange(n_saved):
                max_disp = self._run_block(self.cfg.sample_interval, beta, max_disp, acc_hist,
                                           record_sweeps=True, autotune=sampling_autotune)
                samples_pos[s_idx] = self.pos
                samples_real_pos[s_idx] = self.pos_real
        else:
            # Fallback (no positive interval); still skip saving the first window
            for _ in range(si if si > 0 else 0):
                max_disp = self._run_block(1, beta, max_disp, acc_hist,
                                           record_sweeps=True, autotune=sampling_autotune)
            for s_idx in trange(n_saved):
                max_disp = self._run_block(self.cfg.sample_interval, beta, max_disp, acc_hist,
