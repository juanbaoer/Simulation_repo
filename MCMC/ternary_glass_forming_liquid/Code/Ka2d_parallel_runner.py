...
# ============================== CONFIG ==============================
# Output
OUT_DIR = "/Users/xuhengyuan/Downloads/Notes/Simulation_repo/MCMC/ternary_glass_forming_liquid/Data/Parallel_Ka2d_runs/N43_L6_T0p32"
OVERWRITE = False
SUMMARY_FILENAME = "summary.jsonl"

# Parallelism
WORKERS = 2               # 0 => os.cpu_count()
NUMBA_THREADS = 4         # threads per worker process
...
def _run_one(job: JobSpec, numba_threads: int, disable_tqdm: bool) -> dict:
    ...
    ka = importlib.import_module("Ka2d_random_seed")
    if disable_tqdm:
        ka.trange = range
    ...
    acc = out.get("acceptance", None)
    acc_mean = float(np.mean(acc)) if acc is not None and len(acc) else None
    final_max_disp = out.get("final_max_disp", None)
    final_max_disp = float(final_max_disp) if final_max_disp is not None else None
    energies = out.get("samples_energies", out.get("energies", None))
    e_mean = float(np.mean(energies)) if energies is not None and len(energies) else None
    e_mean_per_particle = (e_mean / float(job.N)) if e_mean is not None else None
    return {
        "job": asdict(job),
        "acceptance_mean": acc_mean,
        "energy_mean": e_mean,
        "energy_mean_per_particle": e_mean_per_particle,
    ...
    for T in Ts:
        ...
        for r in range(1, nrep + 1):
            if series:
                ...
                jid = f"{base}_{r}"
            save_path = os.path.join(out_dir, f"{jid}.npz")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    with ProcessPoolExecutor(max_workers=workers) as ex, open(summary_path, "a", encoding="utf-8") as fsum:
        ...
        for fut in as_completed(futures):
            ...
            epp = rec.get("energy_mean_per_particle", None)
            em = rec.get("energy_mean", None)
            acc_s = f"{acc_mean:.6g}" if acc_mean is not None else "n/a"
            fmd_s = f"{fmd:.6g}" if fmd is not None else "n/a"
            if epp is not None:
                print(
                    f"{jid}: acc={acc_s}  final_max_disp={fmd_s}  "
                    f"mean(E)/N={epp:.8g}  (mean(E)={em:.8g})"
                )
            else:
                print(f"{jid}: acc={acc_s}  final_max_disp={fmd_s}  mean(E)/N=n/a")
            done += 1
